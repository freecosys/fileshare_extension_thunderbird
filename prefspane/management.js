...
let token_web = form.querySelector(`input[name="token_web"]`);
...
browser.storage.local.get([accountId]).then(accountInfo => {
	if (accountId in accountInfo) {
		...
		if ("token_web" in accountInfo[accountId]) {
			token_web.value = accountInfo[accountId].token_web;
		}
	}
});
